...
const CRATER_STAIN_VS = `#version 300 es
...
const CRATER_STAIN_FS = `#version 300 es
precision highp float;
in vec2 v_local;
...
export interface CraterStainPass {
  /** Splat a crater stain at world (x,y) with given radius (meters) and intensity 0..1. */
...
export function createCraterStainPass(
  ...
  const prog = linkProgram(gl, CRATER_STAIN_VS, CRATER_STAIN_FS);
